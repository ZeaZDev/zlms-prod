<%@ Page Language="C#" MasterPageFile="~/Police.Master"  AutoEventWireup="true" CodeBehind="View_detail_report.aspx.cs" Inherits="newweb.QA.View_detail_report" %>
        ...
        <div id="rpBody" class="panel-body" runat="server">
                <asp:GridView ID="gvDP" runat="server" class="table table-striped" AutoGenerateColumns="False" DataSourceID="sqlDP"  ShowFooter="true" OnPreRender="gv1_PreRender" OnRowDataBound="Gv_RowDataBound">
                    <Columns>
                        ...
                        <asp:BoundField DataField="Project" HeaderText="โครงการ" />                        
            ...
            <div id="rpFooter" class="panel-footer" style="text-align: center" runat="server">
                ...
                <asp:SqlDataSource ID="sqlDP" runat="server" ConnectionString="<%$ ConnectionStrings:cdas_conn %>"
                    SelectCommand="SELECT  qr.id,qp.Project,qi.indicator,qsd.qa_standard,qd.Standard_detail,qd.Weight,qr.Score,qd.Weight*qr.Score as multiple FROM [QA_result] qr  left join QA_standard_detail qd on qr.standard_detail_id=qd.id 
left join QA_standard qsd on qsd.id=qd.Standardid left join QA_Indicator qi on qi.id=qsd.qaindicator left join QA_project qp on qp.id=qr.projectid where qr.projectid =@projectid "  >
                   <SelectParameters>
