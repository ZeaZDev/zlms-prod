<%@ Page Language="C#" MasterPageFile="~/Police.Master"  AutoEventWireup="true" CodeBehind="survey_list.aspx.cs" Inherits="newweb.QA_NEW.survey_list" %>
   ...
   <div class="row">
       ...
       <div class="portlet box green">
           <div class="portlet-title">
               <div class="caption">
                    <i class="fa fa-cogs"></i>แบบสอบถาม
               </div>
               <div class="tools">
           ...
           <div class="portlet-body">
             <div class="table-responsive">
              <asp:GridView ID="gvDP" runat="server" class="table table-striped" AutoGenerateColumns="False" DataSourceID="sqlDP"  ShowFooter="false" >
                    <Columns>
                        <asp:TemplateField HeaderText="ชื่อแบบสอบถาม">
                            <ItemTemplate>
                                 <asp:HyperLink ID="lnk" runat="server" Target="_self" Text='<%# Eval("Pollname") %>'
                                            NavigateUrl='<%# "setpoll.aspx?ID=" +Eval("ID") %>'></asp:HyperLink>
                            </ItemTemplate>
                        </asp:TemplateField> 
                        <asp:TemplateField>
                            <ItemTemplate>           
                                  <asp:LinkButton runat="server" ID="LinkButton1" class="btn btn-success " data-toggle="modal"   href="#mdAdd1" data-controls-modal="mdAdd1">
                                   Getlink
                                </asp:LinkButton>                     
           ...
           <asp:SqlDataSource ID="sqlDP" runat="server" ConnectionString="<%$ ConnectionStrings:cdas_conn %>"
                    SelectCommand="SELECT  [id] ,[Pollname] FROM [QA_Poll_ID] c where Active='1'"  >
                </asp:SqlDataSource>
       </div>                             
     ...
     <div id="mdAdd1" class="modal fade in" tabindex="-1" role="dialog" >
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">กรอกรายละเอียด </h4>
                     <h2 id="H1">
                        <label for="fullname">หน่วยงาน </label>
                    ...
                    <h2 id="H2">
                     ...
                     <h2 id="H3">
                        <label for="fullname">ตัวชี้วัด </label>
                         <input type="text" class="form-control" id="Text2" name ="txtNName" placeholder="Full name" runat="server">
                    </h2>  
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <asp:Button ID="Button1" class="btn btn-primary"   runat="server" Text="Add" OnClick="bnAdduser_Click" />
