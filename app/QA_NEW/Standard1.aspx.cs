...
namespace newweb.QA_NEW
{
  public class Standard1 : Page
  {
    ...
    protected void ShowMessage(string Message, Standard1.MessageType type)
    {
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
      ...
      if (linkButton.CommandArgument.ToString().Trim().Length <= 0)
        return;
      this.Response.Redirect("editstd1.aspx?id=" + linkButton.CommandArgument.ToString().Trim());
    }
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      {
        this.ShowMessage("Project Name can not be blank", Standard1.MessageType.Error);
      }
      else
      {
        this.addUser(this.txtNName.Value);
        this.ShowMessage("Project Name  " + this.txtNName.Value + " is added", Standard1.MessageType.Success);
        this.sqlDP.DataBind();
        this.gvDP.DataBind();
        this.clearAddnew();
      }
    ...
    {
      new CConnect().sqlCmd("INSERT INTO [QA_project ] ([Project] ,Stdid,Year,[Active],[Createdate]) VALUES('" + this.txtNName.Value + "','1','" + DateTime.Now.ToString("yyyy") + "','1','" + (object) DateTime.Now + "')");
    }
