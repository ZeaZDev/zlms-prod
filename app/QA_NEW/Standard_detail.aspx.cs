...
// Type: newweb.QA_NEW.Standard_detail
...
{
  public class Standard_detail : Page
  {
    ...
    protected void ShowMessage(string Message, Standard_detail.MessageType type)
    {
    ...
    public void coursename(string classid)
    {
      ...
      connection.Open();
      string cmdText = "SELECT  qi.[id],qp.Project  FROM [QA_standard] qad   inner join QA_Indicator qi on qi.id=qad.qaindicator    inner join QA_project qp on qi.projectid=qp.id  where qad.id=@id";
      ...
      {
        sqlDataReader.Read();
        this.Label1.Text = "ตัวบ่งชี้ของ " + sqlDataReader.GetValue(1).ToString();
      }
      sqlDataReader.Close();
      sqlCommand.Dispose();
      connection.Close();
    ...
    public void delete_course(string id)
    {
      try
      {
        SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["cdas_conn"].ConnectionString);
        connection.Open();
        string cmdText = "UPDATE [QA_standard_detai] SET ACTIVE='0' where id=@id";
        SqlCommand sqlCommand = new SqlCommand(cmdText, connection);
        sqlCommand.CommandText = cmdText;
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      {
        this.ShowMessage("Measure name can not be blank", Standard_detail.MessageType.Error);
      }
      else
      {
        this.addUser(this.txtNName.Value);
        this.ShowMessage("Measure name  " + this.txtNName.Value + " is added", Standard_detail.MessageType.Success);
        this.sqlDP.DataBind();
        this.gvDP.DataBind();
        this.clearAddnew();
      }
    ...
    private void addUser(string user)
    {
      ...
      string sql = "INSERT INTO [QA_standard_detail] [Standardid] ,[Standard_detail] ,[Active],[Createdate],[Weight]) VALUES('" + this.txtNName.Value + "','" + str + "','1','" + (object) DateTime.Now + "','" + this.Text1.Value + "')";
      cconnect.sqlCmd(sql);
    }
