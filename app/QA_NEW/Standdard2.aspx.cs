...
namespace newweb.QA_NEW
{
  public class Standdard2 : Page
  {
    ...
    protected void ShowMessage(string Message, Standdard2.MessageType type)
    {
    ...
    protected void Button6_Click(object sender, EventArgs e)
    {
      ...
      if (linkButton.CommandArgument.ToString().Trim().Length <= 0)
        return;
      this.Response.Redirect("View_detail_report1.aspx?id=" + linkButton.CommandArgument.ToString().Trim());
    }
    ...
    protected void Button7_Click(object sender, EventArgs e)
    {
      ...
      if (linkButton.CommandArgument.ToString().Trim().Length <= 0)
        return;
      this.Response.Redirect("View_report1.aspx?id=" + linkButton.CommandArgument.ToString().Trim());
    }
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
      ...
      if (linkButton.CommandArgument.ToString().Trim().Length <= 0)
        return;
      this.Response.Redirect("editstd2.aspx?id=" + linkButton.CommandArgument.ToString().Trim());
    }
    ...
    public void delete_course(string id)
    {
      try
      {
        ...
        connection.Open();
        string cmdText = "UPDATE [QA_project] SET ACTIVE='0' where id=@id";
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      {
        this.ShowMessage("Project Name can not be blank", Standdard2.MessageType.Error);
      }
      else
      {
        this.addUser(this.txtNName.Value);
        this.ShowMessage("Project Name  " + this.txtNName.Value + " is added", Standdard2.MessageType.Success);
        this.sqlDP.DataBind();
        this.gvDP.DataBind();
        this.clearAddnew();
      }
    ...
    {
      new CConnect().sqlCmd("INSERT INTO [QA_project ] ([Project] ,Stdid,Year,[Active],[Createdate]) VALUES('" + this.txtNName.Value + "','2','" + DateTime.Now.ToString("yyyy") + "','1','" + (object) DateTime.Now + "')");
    }
