...
// Type: newweb.QA_NEW.Poll_detail1
...
{
  public class Poll_detail1 : Page
  {
    ...
    protected void ShowMessage(string Message, Poll_detail1.MessageType type)
    {
    ...
    public void delete_course(string id)
    {
      try
      {
        ...
        connection.Open();
        string cmdText = "UPDATE [QA_Poll_detail1] SET ACTIVE='0' where id=@id";
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      {
        this.ShowMessage("Measure name can not be blank", Poll_detail1.MessageType.Error);
      }
      else
      {
        this.addUser(this.txtNName.Value);
        this.ShowMessage("Measure name  " + this.txtNName.Value + " is added", Poll_detail1.MessageType.Success);
        this.sqlDP.DataBind();
        this.gvDP.DataBind();
        this.clearAddnew();
        this.Page.Response.Redirect(this.Page.Request.Url.AbsoluteUri);
    ...
    private void addUser(string user)
    {
      ...
      string sql = "INSERT INTO [QA_Poll_detail1] ([Standard_detail],[pollid] ,[Active],[Createdate],[Weight]) VALUES('" + this.txtNName.Value + "','" + str + "','1','" + (object) DateTime.Now + "','" + this.Text1.Value + "')";
      cconnect.sqlCmd(sql);
    }
