...
namespace newweb.QA_NEW
{
  public class Standard : Page
  {
    ...
    protected void ShowMessage(string Message, Standard.MessageType type)
    {
    ...
    public void coursename(string classid)
    {
      ...
      connection.Open();
      string cmdText = "SELECT  qi.[id],qp.Project  FROM [QA_standard] qad   inner join QA_Indicator qi on qi.id=qad.qaindicator    inner join QA_project qp on qi.projectid=qp.id  where qi.id=@id group by qi.[id],qp.Project";
      ...
      {
        sqlDataReader.Read();
        this.Label1.Text = "ด้านของ " + sqlDataReader.GetValue(1).ToString();
      }
      sqlDataReader.Close();
      sqlCommand.Dispose();
      connection.Close();
    ...
    public void delete_course(string id)
    {
      try
      {
        SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["cdas_conn"].ConnectionString);
        connection.Open();
        string cmdText = "UPDATE QA_standard SET ACTIVE='0' where id=@id";
        SqlCommand sqlCommand = new SqlCommand(cmdText, connection);
        sqlCommand.CommandText = cmdText;
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      {
        this.ShowMessage("QA Standard  Group can not be blank", Standard.MessageType.Error);
      }
      else
      {
        this.addUser(this.txtNName.Value);
        this.ShowMessage("QA Standard Group  " + this.txtNName.Value + " is added", Standard.MessageType.Success);
        this.sqlDP.DataBind();
        this.gvDP.DataBind();
        this.clearAddnew();
      }
    ...
    private void addUser(string user)
    {
      ...
      string sql = "INSERT INTO [QA_standard] ([qaindicator],[qa_standard],[Active],[Createdate]) VALUES('" + str + "','" + this.txtNName.Value + "','1','" + (object) DateTime.Now + "')";
      cconnect.sqlCmd(sql);
    }
