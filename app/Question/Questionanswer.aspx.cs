...
// Type: newweb.Question.Questionanswer
...
{
  public class Questionanswer : Page
  {
    ...
    protected HtmlInputText detailanswer;
    protected Button bnAdduser;

    protected void ShowMessage(string Message, Questionanswer.MessageType type)
    {
    ...
    public void coursename(string classid)
    {
      ...
      connection.Open();
      string cmdText = "select q.Question_name,qd.Question_detail,qg.QuestiongroupName from Question_detail qd inner join Questiongroup qg on qg.id=qd.Question_group inner join Question q on q.id=qd.Question_id where qd.id=@id";
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
      ...
      if (linkButton.CommandArgument.ToString().Trim().Length <= 0)
        return;
      this.Response.Redirect("Questionitemupload.aspx?id=" + linkButton.CommandArgument.ToString().Trim());
    }
    ...
    public void delete_course(string id)
    {
      try
      {
        SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["cdas_conn"].ConnectionString);
        connection.Open();
        string cmdText = "update [Question_data] set active='0' where id=@id";
        SqlCommand sqlCommand = new SqlCommand(cmdText, connection);
        sqlCommand.CommandText = cmdText;
    ...
    protected void bnAdduser_Click(object sender, EventArgs e)
    {
      ...
      this.ShowMessage("Question Data " + this.txtNName.Value + " is added", Questionanswer.MessageType.Success);
      this.sqlDP.DataBind();
      this.gvDP.DataBind();
      this.clearAddnew();
    }
    ...
    private void addUser(string user)
    {
      ...
      if (this.CheckBox1.Checked)
        str2 = "1";
      string sql = "INSERT INTO [Question_data] ([Question_detail_id],[Question_data],[Iscorrect],[Active],[Detail_answer]) VALUES('" + str1 + "','" + user + "','" + str2 + "','1','" + this.detailanswer.Value + "')";
      cconnect.sqlCmd(sql);
    }
